...
export class DiffUtils {
    ...
    static async pdfPageToPng(doc: PDFDocumentProxy, pageNumber: number, scale?: number): Promise<Buffer> {
    ...
    static diffPng(left: Buffer, right: Buffer, config?: {threshold?: number}) {
        return new Promise<{ndiff: number, diff?: Buffer}>((resolve, reject) => {
            try {
                ...
                let numDiffPixels = pixelmatch(rightpng.data, leftpng.data, diffpng.data, width, height, {
                    threshold: threshold
                });
